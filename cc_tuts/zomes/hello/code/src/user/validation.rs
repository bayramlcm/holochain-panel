...
pub fn validate_entry_create(entry: UserEntry, validation_data: hdk::ValidationData) -> Result<(), String> {
    hdk::debug(format!("validate_entry_modify_new_entry: {:?}", entry)).ok();
    hdk::debug(format!("validate_entry_modify_validation_entry: {:?}", validation_data)).ok();
    Ok(())
}

pub fn validate_entry_modify(new_entry: UserEntry, old_entry: UserEntry, old_entry_header: ChainHeader, validation_data: hdk::ValidationData) -> Result<(), String> {
    ...
    if let (Some(o), Some(p)) = (old_entry_header.provenances().get(0), validation_data.package.chain_header.provenances().get(0)) {
        if o.source() == p.source() {
            ...
            Err("Yabancı kullanıcı güncelleme yapmaya çalışıyor".to_string())
        }
    } else {
        Err("validation_data eşleşme hatası".to_string())
    }
}

pub fn validate_entry_delete(old_entry: UserEntry, old_entry_header: ChainHeader, validation_data: hdk::ValidationData) -> Result<(), String> {
    ...
    if let (Some(o), Some(p)) = (old_entry_header.provenances().get(0), validation_data.package.chain_header.provenances().get(0)) {
        if o.source() == p.source() {
            ...
            Err("Yabancı kullanıcı silmeye çalışıyor".to_string())
        }
    } else {
        Err("validation_data eşleşme hatası".to_string())
    }
